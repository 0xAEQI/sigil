...
impl QuestBoard {
    ...
    fn persist(&self, bead: &Quest) -> Result<()> {
    ...
    pub fn create(&mut self, prefix: &str, subject: &str) -> Result<Quest> {
        let seq = self.sequences.entry(prefix.to_string()).or_insert(0);
        *seq += 1;
        let id = QuestId::root(prefix, *seq);

        let bead = Quest::new(id, subject);
        self.persist(&bead)?;
    ...
    pub fn create_child(&mut self, parent_id: &QuestId, subject: &str) -> Result<Quest> {
        ...
        let mut bead = Quest::new(id, subject);
    ...
    pub fn all(&self) -> Vec<&Quest> {
        let mut beads: Vec<&Quest> = self.beads.values().collect();
        beads.sort_by(|a, b| a.created_at.cmp(&b.created_at));
        beads
    }
