...
use realm_core::traits::{ToolResult, ToolSpec};
use realm_core::traits::Tool;
use realm_quests::{QuestBoard, Priority};
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let bead = store.update(id, |b| {
            if let Some(s) = status {
                b.status = match s {
                    "in_progress" => realm_quests::QuestStatus::InProgress,
                    "done" => realm_quests::QuestStatus::Done,
                    "blocked" => realm_quests::QuestStatus::Blocked,
                    "cancelled" => realm_quests::QuestStatus::Cancelled,
                    _ => realm_quests::QuestStatus::Pending,
                };
            }
