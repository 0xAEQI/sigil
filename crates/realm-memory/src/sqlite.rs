...
use realm_core::traits::{Embedder, Memory, MemoryCategory, MemoryEntry, MemoryQuery, MemoryScope};
...
impl SqliteMemory {
    ...
    fn migrate(conn: &Connection) -> Result<()> {
        ...
        if !has_scope {
            ...
            debug!("migrated memories table: added scope + companion_id columns");
        }
    ...
    fn bm25_search(conn: &Connection, query: &MemoryQuery, limit: usize) -> Result<Vec<(MemRow, f64)>> {
        ...
        if let Some(ref cid) = query.companion_id {
            conditions.push(format!("m.companion_id = ?{idx}"));
            params.push(Box::new(cid.clone()));
            idx += 1;
        } else if query.scope == Some(MemoryScope::Domain) {
            conditions.push("m.companion_id IS NULL".to_string());
        }

        let where_clause = conditions.join(" AND ");

        let sql = format!(
            "SELECT m.id, m.key, m.content, m.category, m.scope, m.companion_id,
        ...
        let rows = stmt
            .query_map(param_refs.as_slice(), |row| {
                ...
                let companion_id: Option<String> = row.get(5)?;
                let created_at: String = row.get(6)?;
                let session_id: Option<String> = row.get(7)?;
                let bm25: f64 = row.get(8)?;
                Ok((MemRow { id, key, content, cat_str, scope_str, companion_id, created_at, session_id }, bm25))
            })?
            .filter_map(|r| r.ok())
            .collect();
    ...
    fn fetch_by_ids(conn: &Connection, ids: &[String]) -> Vec<MemRow> {
        ...
        let placeholders = ids
            ...
            .join(",");
        let sql = format!(
            "SELECT id, key, content, category, scope, companion_id, created_at, session_id
...
impl Memory for SqliteMemory {
    async fn store(
...

        {
            let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("lock: {e}"))?;
            conn.execute(
                "INSERT INTO memories (id, key, content, category, scope, companion_id, created_at)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                rusqlite::params![id, key, content, cat, scope_str, companion_id, now],
            )?;
        }

        debug!(id = %id, key = %key, scope = %scope_str, companion_id = ?companion_id, "memory stored");
...
mod tests {
    ...
    #[tokio::test]
    async fn test_companion_scoped_memory() {
        let dir = tempfile::TempDir::new().unwrap();
        let db_path = dir.path().join("test_companion.db");
        let mem = SqliteMemory::open(&db_path, 30.0).unwrap();

        mem.store("shared-fact", "The API runs on port 8080", MemoryCategory::Fact, MemoryScope::Domain, None)
            .await.unwrap();
        mem.store("guardian-note", "Risk tolerance is low for this user", MemoryCategory::Preference, MemoryScope::Companion, Some("guardian-001"))
            .await.unwrap();
        mem.store("librarian-note", "User prefers detailed explanations", MemoryCategory::Preference, MemoryScope::Companion, Some("librarian-001"))
            .await.unwrap();

        let guardian_query = MemoryQuery::new("risk tolerance", 10).with_companion("guardian-001");
        let results = mem.search(&guardian_query).await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].companion_id.as_deref(), Some("guardian-001"));

        let librarian_query = MemoryQuery::new("risk tolerance", 10).with_companion("librarian-001");
        ...
        assert_eq!(results.len(), 1);
        assert!(results[0].companion_id.is_none());
    }

    #[tokio::test]
    async fn test_realm_scoped_memory() {
        let dir = tempfile::TempDir::new().unwrap();
        let db_path = dir.path().join("test_realm.db");
        let mem = SqliteMemory::open(&db_path, 30.0).unwrap();

        mem.store("strategic-pref", "Always prefer Rust over Python for new services", MemoryCategory::Preference, MemoryScope::Realm, None)
            .await.unwrap();
        mem.store("domain-fact", "The trading engine uses 50us tick", MemoryCategory::Fact, MemoryScope::Domain, None)
            .await.unwrap();

        let realm_query = MemoryQuery::new("Rust Python", 10).with_scope(MemoryScope::Realm);
        let results = mem.search(&realm_query).await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].scope, MemoryScope::Realm);
    ...
    async fn test_migration_on_existing_db() {
        ...
        mem.store("new-fact", "New data with scope", MemoryCategory::Fact, MemoryScope::Companion, Some("comp-1"))
            .await.unwrap();

        let results = mem.search(&MemoryQuery::new("New data scope", 10).with_companion("comp-1")).await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].companion_id.as_deref(), Some("comp-1"));
    }
    ...
    #[async_trait]
    impl realm_core::traits::Embedder for MockEmbedder {
