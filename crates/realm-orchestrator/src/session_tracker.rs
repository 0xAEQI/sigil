...
use realm_core::traits::{Channel, OutgoingMessage};
...
/// - Periodic sprint check-ins while spirits are working
...
impl SessionTracker {
    pub async fn run(self, shutdown: Arc<Notify>) {
        ...
        loop {
            ...
            let (total_working, total_pending, active_domains) = aggregate_status(&status);
            ...
            let msg: Option<String> = if can_send {
                ...
                // 2. Transition: idle → active (queue filled after empty).
                    ...
                    Some(format!(
                        "🚀 Spirits awakened — {} {} queued across {}. Session: {}.",
                        total_pending + total_working,
                        if total_pending + total_working == 1 { "quest" } else { "quests" },
                        active_domains,
                        fmt_duration(elapsed)
                    ))
                ...
                // 3. Transition: active → idle (queue emptied).
                    ...
                    Some(format!(
                        "💤 Queue empty — all spirits at rest. Session: {}.",
                        fmt_duration(elapsed)
                    ))
                        ...
                        .map_or(true, |t| t.elapsed() >= self.checkin_interval)
                {
                    ...
                    Some(format!(
                        ...
                        total_working,
                        if total_working == 1 { "spirit" } else { "spirits" },
                        active_domains,
                        fmt_duration(elapsed)
                    ))
                        ...
                        .map_or(true, |t| t.elapsed() >= self.alarm_interval)
                {
                    last_alarm = Some(Instant::now());
                    Some(format!(
                        "🔔 Realm idle — {} elapsed. Ready for your next command, Architect.",
                        fmt_duration(elapsed)
                    ))
                } else {
                    None
...
/// Summarise total working/pending counts and active domain names.
fn aggregate_status(
    ...
    for d in &status.domains {
        let working = d.spirits_working + d.spirits_bonded;
        let pending = d.open_quests + d.ready_quests;
    ...
    let domains_str = match active_names.len() {
        0 => "no domains".to_string(),
        1 => active_names[0].to_string(),
        n => format!("{n} domains"),
    };

    (total_working, total_pending, domains_str)
}
