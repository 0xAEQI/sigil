...
/// A raid tracks work across multiple rigs.
...
impl Raid {
    pub fn new(name: &str, beads: Vec<(QuestId, String)>) -> Self {
        let id = format!("raid-{}", uuid::Uuid::new_v4().as_simple());
        Self {
            id,
            name: name.to_string(),
            beads: beads.into_iter().map(|(quest_id, rig)| RaidBead {
                quest_id,
                rig,
                closed: false,
            }).collect(),
...
impl RaidStore {
    ...
    /// Create a new raid.
    pub fn create(&mut self, name: &str, beads: Vec<(QuestId, String)>) -> Result<&Raid> {
        let raid = Raid::new(name, beads);
        info!(id = %raid.id, name = %name, beads = raid.beads.len(), "raid created");
    ...
    pub fn mark_bead_closed(&mut self, quest_id: &QuestId) -> Result<Vec<String>> {
    ...
    /// Get a raid by ID.
    pub fn get(&self, id: &str) -> Option<&Raid> {
        self.raids.iter().find(|c| c.id == id)
    }

    /// List active (unclosed) raids.
    pub fn active(&self) -> Vec<&Raid> {
