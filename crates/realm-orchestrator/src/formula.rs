//! Formula — Reusable workflow templates that generate quest DAGs.
//!
//! A Formula is a TOML template defining a sequence of steps with variable
...
/// Discover formulas from shared + domain directories (domain overrides shared).
pub fn discover_formulas(shared_dir: &Path, domain_dir: &Path) -> Result<Vec<Formula>> {
    let mut formulas = Vec::new();

    let shared_mol = shared_dir.join("molecules");
    if shared_mol.exists() {
        formulas.extend(Formula::load_all(&shared_mol)?);
    }

    let domain_mol = domain_dir.join("molecules");
    if domain_mol.exists() {
        for df in Formula::load_all(&domain_mol)? {
...
labels = ["review"]
"#;
        let formula: Formula = toml::from_str(toml_str).unwrap();
