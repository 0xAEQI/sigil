...
/// External checkpoint for a spirit's work-in-progress.
///
...
impl SpiritCheckpoint {
    ...
    pub fn with_spirit_name(mut self, name: impl Into<String>) -> Self {
        self.spirit_name = Some(name.into());
        self
    }
    ...
    /// Standard checkpoint file path for a given quest in a rig's .sigil directory.
    ///
    /// Layout: `<rig_dir>/.sigil/checkpoints/<quest_id>.json`
    pub fn path_for_quest(rig_dir: &Path, quest_id: &str) -> PathBuf {
        rig_dir
            .join(".sigil")
            .join("checkpoints")
...
mod tests {
    ...
    fn test_checkpoint_write_and_read() {
        ...
        let checkpoint = SpiritCheckpoint {
            quest_id: Some("sg-042".to_string()),
            spirit_name: Some("sigil-worker-1".to_string()),
            modified_files: vec![
        ...
        let loaded = SpiritCheckpoint::read(&cp_path).unwrap().unwrap();
        assert_eq!(loaded.quest_id, Some("sg-042".to_string()));
        assert_eq!(loaded.spirit_name, Some("sigil-worker-1".to_string()));
    ...
    fn test_checkpoint_read_missing_file() {
        ...
        let result = SpiritCheckpoint::read(&cp_path).unwrap();
    ...
    fn test_checkpoint_remove() {
        ...
        SpiritCheckpoint::remove(&cp_path).unwrap();
        assert!(!cp_path.exists());
    ...
    fn test_checkpoint_path_for_quest() {
        let path = SpiritCheckpoint::path_for_quest(Path::new("/home/dev/rigs/sigil"), "sg-001");
        assert_eq!(
            path,
            PathBuf::from("/home/dev/rigs/sigil/.sigil/checkpoints/sg-001.json")
        );
    }
    ...
    fn test_checkpoint_builder_methods() {
        ...
        let cp = checkpoint
            ...
            .with_spirit_name("sigil-worker-1")
            .with_progress_notes("Did some work");

        assert_eq!(cp.quest_id, Some("sg-001".to_string()));
        assert_eq!(cp.spirit_name, Some("sigil-worker-1".to_string()));
    ...
    fn test_capture_in_git_repo() {
        ...
        let checkpoint = SpiritCheckpoint::capture(workdir).unwrap();
