...
impl ContextBudget {
    ...
    pub fn budget_checkpoints(&self, checkpoints: &[realm_quests::Checkpoint]) -> String {
        ...
        if checkpoints.len() <= self.max_checkpoint_count {
            ...
            for cp in &checkpoints[..split] {
                ...
                out.push_str(&format!(
                    "- {} ({} turns, ${:.4}): {}\n",
                    cp.spirit, cp.turns_used, cp.cost_usd, line
                ));
            }
            out.push('\n');
    ...
    pub fn apply_to_identity(&self, identity: &realm_core::Identity) -> String {
        ...
        if let Some(ref soul) = identity.soul {
...
mod tests {
    ...
    fn test_budget_checkpoints_few() {
        ...
        let cps = vec![realm_quests::Checkpoint {
