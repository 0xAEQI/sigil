...
pub struct FateJob {
    ...
    pub rig: String,
    pub prompt: String,
    /// If true, spawn an isolated worker. If false, enqueue for next pulse.
    #[serde(default)]
    pub isolated: bool,
...
pub struct FateStore {
    path: PathBuf,
    pub jobs: Vec<FateJob>,
}
...
impl FateStore {
    ...
    /// Add a cron job.
    pub fn add(&mut self, job: FateJob) -> Result<()> {
        if self.jobs.iter().any(|j| j.name == job.name) {
            anyhow::bail!("cron job '{}' already exists", job.name);
        }
        info!(name = %job.name, rig = %job.rig, "cron job added");
        self.jobs.push(job);
        self.save()
    }
    ...
    pub fn due_jobs(&self) -> Vec<&FateJob> {
        self.jobs
            .iter()
