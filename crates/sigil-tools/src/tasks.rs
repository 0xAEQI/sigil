...
use sigil_tasks::{TaskBoard, Priority};
...
impl Tool for TaskCreateTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Create a new task with a subject and optional description/priority.".to_string(),
...
impl Tool for TaskUpdateTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        Ok(ToolResult::success(format!("Updated {} [{}] {}", task.id, task.status, task.subject)))
    }
...
impl Tool for TaskCloseTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        Ok(ToolResult::success(format!("Closed {} — {}", task.id, task.subject)))
    }
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        if let Some(task) = store.get(id) {
            let deps = if task.depends_on.is_empty() {
                ...
                task.depends_on.iter().map(|d| d.0.as_str()).collect::<Vec<_>>().join(", ")
            };
            let blocks = if task.blocks.is_empty() {
                "none".to_string()
            } else {
                task.blocks.iter().map(|b| b.0.as_str()).collect::<Vec<_>>().join(", ")
            };
            let assignee = task.assignee.as_deref().unwrap_or("unassigned");

            let output = format!(
                "ID: {}\nSubject: {}\nStatus: {}\nPriority: {}\nAssignee: {}\nDescription: {}\nDepends on: {}\nBlocks: {}\nCreated: {}",
                task.id, task.subject, task.status, task.priority, assignee,
                if task.description.is_empty() { "(none)" } else { &task.description },
                deps, blocks, task.created_at
            );
            Ok(ToolResult::success(output))
        } else {
