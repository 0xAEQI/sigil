...
impl Blackboard {
    pub fn open(path: &Path, transient_ttl_hours: u64, durable_ttl_days: u64) -> Result<Self> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .with_context(|| format!("failed to create blackboard dir: {}", parent.display()))?;
        }
        ...
        Ok(Self { conn: Mutex::new(conn), transient_ttl_hours, durable_ttl_days })
    }

    /// Post a new entry to the blackboard.
    pub fn post(&self, key: &str, content: &str, agent: &str, project: &str, tags: &[String], durability: EntryDurability) -> Result<BlackboardEntry> {
        let now = Utc::now();
        let expires_at = match durability {
            EntryDurability::Transient => now + chrono::Duration::hours(self.transient_ttl_hours as i64),
        ...
        let dur_str = serde_json::to_value(entry.durability)?
            .as_str().unwrap_or("transient").to_string();
    ...
    pub fn query(&self, project: &str, tags: &[String], limit: u32) -> Result<Vec<BlackboardEntry>> {
        let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("lock poisoned: {e}"))?;
        ...
        let mut matched: Vec<BlackboardEntry> = entries.into_iter()
    ...
    pub fn prune_expired(&self) -> Result<u64> {
        let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("lock poisoned: {e}"))?;
        let now = Utc::now().to_rfc3339();
        let count = conn.execute("DELETE FROM sigil_blackboard WHERE expires_at <= ?1", rusqlite::params![now])?;
        Ok(count as u64)
    }
    ...
    fn list_project_internal(&self, conn: &Connection, project: &str, limit: u32, now: &str) -> Result<Vec<BlackboardEntry>> {
        let mut stmt = conn.prepare(
            "SELECT id, key, content, agent, project, tags_json, durability, created_at, expires_at
        ...
        let entries: Vec<BlackboardEntry> = stmt.query_map(
            rusqlite::params![project, now, limit],
            Self::row_to_entry,
    ...
    fn row_to_entry(row: &rusqlite::Row) -> rusqlite::Result<BlackboardEntry> {
        ...
        let durability: EntryDurability = serde_json::from_value(
        ...
        Ok(BlackboardEntry { id, key, content, agent, project, tags, durability, created_at, expires_at })
    }
}
...
mod tests {
    ...
    fn test_post_and_query() {
        let (bb, _dir) = temp_bb();
        bb.post("api-endpoint", "POST /api/v2/users", "worker-1", "proj-a", &["api".to_string()], EntryDurability::Transient).unwrap();
        bb.post("db-schema", "users table has uuid PK", "worker-2", "proj-a", &["database".to_string()], EntryDurability::Durable).unwrap();
    ...
    fn test_tag_relevance() {
        let (bb, _dir) = temp_bb();
        bb.post("k1", "content1", "a", "p", &["rust".to_string(), "backend".to_string()], EntryDurability::Transient).unwrap();
        bb.post("k2", "content2", "a", "p", &["python".to_string()], EntryDurability::Transient).unwrap();
    ...
    fn test_prune_expired() {
        ...
        bb.post("k1", "c1", "a", "p", &[], EntryDurability::Transient).unwrap();

        // Manually expire it by updating the DB
        {
            let conn = bb.conn.lock().unwrap();
            let expired = (Utc::now() - chrono::Duration::hours(1)).to_rfc3339();
            conn.execute("UPDATE sigil_blackboard SET expires_at = ?1", rusqlite::params![expired]).unwrap();
        }
    ...
    fn test_get_by_key() {
        let (bb, _dir) = temp_bb();
        bb.post("config-path", "/etc/app.toml", "w1", "proj", &[], EntryDurability::Durable).unwrap();
