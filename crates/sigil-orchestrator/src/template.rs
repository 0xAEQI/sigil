//! Template — Reusable workflow templates that generate task DAGs.
//!
//! A Template is a TOML template defining a sequence of steps with variable
//! substitution. When "poured", it creates a parent task with child tasks
...
impl Template {
    ...
    /// Pour: resolve variables and produce task structure.
    pub fn pour(&self, vars: &HashMap<String, String>) -> Result<PouredTemplate> {
...
/// Discover templates from shared + project directories (project overrides shared).
pub fn discover_templates(shared_dir: &Path, project_dir: &Path) -> Result<Vec<Template>> {
    let mut templates = Vec::new();

    let shared_pipelines = shared_dir.join("pipelines");
    let shared_pipelines = if shared_pipelines.exists() {
        shared_pipelines
    } else {
        shared_dir.join("molecules")
    };
    if shared_pipelines.exists() {
        templates.extend(Template::load_all(&shared_pipelines)?);
    }

    let project_pipelines = project_dir.join("pipelines");
    let project_pipelines = if project_pipelines.exists() {
        project_pipelines
    } else {
        project_dir.join("molecules")
    };
    if project_pipelines.exists() {
        for df in Template::load_all(&project_pipelines)? {
            if let Some(pos) = templates.iter().position(|f| f.name == df.name) {
                templates[pos] = df;
            } else {
                templates.push(df);
            }
...
mod tests {
    ...
    #[test]
    fn test_template_parse_and_pour() {
        let toml_str = r#"
...
"#;
        let template: Template = toml::from_str(toml_str).unwrap();
        assert!(template.pour(&HashMap::new()).is_err());
    }
}
