...
///
/// Runs on a configurable interval. Reads identity files from the project directory,
...
/// PREFERENCES.md inside the project directory. No sub-agents are spawned; the call
/// is a single synchronous `provider.chat()` invocation.
pub struct Reflection {
    pub project_name: String,
    pub interval_secs: u64,
    /// Project directory where identity files and `.sigil/` data live.
    pub project_dir: PathBuf,
    pub provider: Arc<dyn Provider>,
    pub model: String,
    /// Optional SQLite memory for cross-system recall.
...
impl Reflection {
    pub fn new(
        ...
        let state_path = project_dir.join(".sigil/reflection-state.json");
        Self {
            project_name,
            interval_secs,
            project_dir,
    ...
    /// Set the memory backend for cross-system recall.
    ...
    pub async fn run(&mut self) -> Result<String> {
        debug!(project = %self.project_name, "starting reflection cycle");
        ...
        if !drift_detected {
            debug!(project = %self.project_name, "no drift detected — skipping reflection");
        ...
        // Query SQLite memory for recent insights to inform reflection.
        let sqlite_insights = if let Some(ref mem) = self.memory {
            let query = sigil_core::traits::MemoryQuery::new(
                format!("reflection insights for {}", self.project_name),
                5,
            )
        ...
        let prompt = build_prompt(&self.project_name, &file_contents, &sqlite_insights);
        ...
        let Some(text) = response.content else {
            warn!(project = %self.project_name, "reflection LLM returned empty response");
            self.last_run = Some(std::time::Instant::now());
            return Ok("empty LLM response".to_string());
        };

        if text.trim() == "NO_CHANGES" {
            info!(project = %self.project_name, "reflection: no changes needed");
            ...
            if let Err(e) = state.save(&self.state_path) {
                warn!(project = %self.project_name, error = %e, "failed to save reflection state");
            }
            self.last_run = Some(std::time::Instant::now());
        ...
        // Write reflection summary back to SQLite memory for cross-system recall.
        if !updated_files.is_empty()
            && let Some(ref mem) = self.memory
        {
            let summary_text = format!(
                "Reflection updated: {}. Project: {}",
                updated_files.join(", "),
                self.project_name
            );
        ...
        info!(project = %self.project_name, summary = %summary, "reflection cycle complete");
        Ok(summary)
    }
}
...
fn build_prompt(project_name: &str, files: &HashMap<String, String>, sqlite_insights: &str) -> String {
    ...
    if !sqlite_insights.is_empty() {
        prompt.push_str("## Recent Insights from SQLite Memory\n\n");
        prompt.push_str(sqlite_insights);
        prompt.push_str("\n\n");
    }
...
mod tests {
    ...
    fn test_drift_detection_logic() {
        ...
        fp1.insert("PERSONA.md".to_string(), fnv1a("original content"));
