...
pub(crate) fn render_user_service(config_path: &Path) -> Result<String> {
    ...
    let exe = std::env::current_exe().context("failed to locate current sigil executable")?;
...
pub(crate) fn install_user_service(
    ...
    if systemctl_exists() {
        ...
        if let Err(e) = run_systemctl_user(["enable", "sigil.service"]) {
            warnings.push(format!("systemctl --user enable failed: {e}"));
        }
        if start && let Err(e) = run_systemctl_user(["start", "sigil.service"]) {
...
pub(crate) fn uninstall_user_service(stop: bool) -> Result<(Option<PathBuf>, Vec<String>)> {
    ...
    if systemctl_exists() {
        if stop && let Err(e) = run_systemctl_user(["stop", "sigil.service"]) {
            warnings.push(format!("systemctl --user stop failed: {e}"));
        }
        if let Err(e) = run_systemctl_user(["disable", "sigil.service"]) {
...
fn user_service_path() -> Result<PathBuf> {
    ...
    Ok(home.join(".config/systemd/user/sigil.service"))
}
